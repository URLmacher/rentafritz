...
function navOpen() {
	...
	if (x.className === "navigation") {
		...
		if(!$('.navigation').hasClass('open')){
			...
			$('.title-siedl').addClass("hide");
		}else{
			$('.navigation').removeClass("open");
			$('.title-siedl').removeClass("hide");
		}
	});
	...
	$(function(){
		$('#time-start').timepicker({appendTo: function($el){
			return $el.parent()
		},
		'timeFormat': 'H:i',
		'showDuration': true
	...
	$(function(){
		$('#time-end').timepicker({appendTo: function($eel){
			return $eel.parent()
		},
		'timeFormat': 'H:i',
		'showDuration': true
	...
	*/
	...
	$('#button').click(function(){
		...
		if(isNaN(price)){
			...
			if(hours == 1 & !(selectedText == "Palettenhubwagen" || selectedText == "Schmutzwasserpumpe" ) ){
	...
	$(".comment").eq(random).addClass("active-slide");
	...
	((selectedText == "Palettenhubwagen")&(hours <= 4)){
...
*/
	$('.open-box').click(function(){
		$(this).parent().find('.mehr-info').removeClass('hide');
		$(this).parent().find('.wenig-info').addClass('hide');
		$(this).addClass('hide');
		$(this).parent().find('.close-box').removeClass('hide');
	});
	$('.close-box').click(function(){
		$(this).parent().find('.mehr-info').addClass('hide');
		$(this).parent().find('.wenig-info').removeClass('hide');
		$(this).addClass('hide');
		$(this).parent().find('.open-box').removeClass('hide');
	});
});